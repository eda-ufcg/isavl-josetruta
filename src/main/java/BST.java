...
public class BST {
    ...
    private boolean isAVL(Node node) {
        ...
        else if (Math.abs(this.balance(node)) > 1) return false;
        else if (node.right != null) return this.isAVL(node.right);
        else if (node.left != null) return this.isAVL(node.left);
        else return true;
    }
    ...
    private int balance(Node node) {
        if (node != null) return this.height(node.left) - this.height(node.right);
        return 0;
    }
